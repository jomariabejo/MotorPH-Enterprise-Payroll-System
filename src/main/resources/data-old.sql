...
CREATE TABLE IF NOT EXISTS `leave_request` (
  ...
  `DateRequested` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'The creation date of the leave request.',
...
INSERT INTO `leave_request` (`LeaveRequestID`, `EmployeeID`, `LeaveTypeID`, `DateRequested`, `StartDate`, `EndDate`, `Status`, `AdminApprovalEmployeeID`, `AdminApprovalDate`, `Description`) VALUES
	(25, 4, 1, '2024-08-03 15:14:12', '2024-08-03', '2024-08-03', 'Pending', NULL, NULL, ''),
	(26, 4, 1, '2024-08-03 15:15:46', '2024-08-05', '2024-08-06', 'Pending', NULL, NULL, ''),
	(27, 4, 1, '2024-08-03 15:25:53', '2024-08-30', '2024-08-31', 'Pending', NULL, NULL, ''),
	(28, 4, 1, '2024-08-04 09:47:15', '2024-08-04', '2024-08-04', 'Approved', NULL, NULL, ''),
	(29, 4, 1, '2024-08-04 21:14:02', '2024-08-19', '2024-08-23', 'Pending', NULL, NULL, ''),
	(30, 4, 3, '2024-08-04 21:14:30', '2024-09-24', '2024-09-25', 'Pending', NULL, NULL, '');
...
CREATE TABLE IF NOT EXISTS `overtime_request` (
  ...
  `DateRequested` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Timestamp when the overtime request was submitted.',
...
-- Dumping data for table payroll_system.user: ~1 rows (approximately)
INSERT INTO `user` (`UserID`, `Username`, `Password`, `Email`, `FullName`, `RoleID`, `Status`, `EmployeeId`) VALUES
	(1, 'biancasofia', 'helloworld', 'biancake@gmail.com', 'Biance Sofia', 1, 'Active', 4);
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vw_active_users` AS select `u`.`UserID` AS `UserID`,`u`.`Username` AS `Username`,`u`.`Email` AS `Email`,`u`.`FullName` AS `FullName`,`r`.`RoleName` AS `RoleName`,`u`.`Status` AS `Status` from (`users` `u` join `role` `r` on((`u`.`RoleID` = `r`.`RoleID`))) where (`u`.`Status` = 'Active');
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vw_overtime_requests` AS select `o`.`RequestID` AS `RequestID`,`o`.`EmployeeID` AS `EmployeeID`,`e`.`FirstName` AS `FirstName`,`e`.`LastName` AS `LastName`,`o`.`DateRequested` AS `DateRequested`,`o`.`Date` AS `Date`,`o`.`HoursRequested` AS `HoursRequested`,`o`.`Status` AS `Status`,`o`.`HRRemarks` AS `HR Remarks` from (`overtime_request` `o` join `employee` `e` on((`o`.`EmployeeID` = `e`.`EmployeeNumber`)));
