...
@Getter
@Setter
public class PhilhealthContributionRateFormController {

    private PhilhealthContributionRateController rateController;
    private PhilhealthContributionRate rate;
    ...
    private void createRate() {
        PhilhealthContributionRate newRate = new PhilhealthContributionRate();
        mapFieldsToRate(newRate);
        
        rateController.getPayrollAdministratorNavigationController()
                .getMainViewController().getServiceFactory()
                .getPhilhealthContributionRateService().saveRate(newRate);

        CustomAlert alert = new CustomAlert(
                Alert.AlertType.INFORMATION,
                "Success",
                "Philhealth contribution rate created successfully."
        );
        alert.showAndWait();
    ...
    private void updateRate() {
        ...
        rateController.getPayrollAdministratorNavigationController()
                .getMainViewController().getServiceFactory()
                .getPhilhealthContributionRateService().updateRate(rate);

        CustomAlert alert = new CustomAlert(
                Alert.AlertType.INFORMATION,
                "Success",
                "Philhealth contribution rate updated successfully."
        );
        alert.showAndWait();
    ...
    private void mapFieldsToRate(PhilhealthContributionRate rate) {
